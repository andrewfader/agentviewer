//! Text-to-speech through espeak-ng, plus the amplitude envelope that drives
//! mouth overlays.
//!
//! Microsoft Agent got viseme timings from the SAPI 4 engine. espeak-ng does
//! not expose those, so the mouth is driven from the loudness of the rendered
//! audio instead: loud passages open the mouth wide, silence closes it. That
//! tracks speech closely enough to read as lip sync.
...
/// Renders `text` to audio using the character's voice settings where present.
pub fn synthesize(text: &str, voice: Option<&VoiceInfo>) -> Result<Speech, String> {
    let text = text.trim();
    if text.is_empty() {
